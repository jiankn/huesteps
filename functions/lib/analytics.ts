...
let engagementSchemaPromise: Promise<void> | null = null;

/**
 * Keeps engagement analytics deployable when a Pages Git build lands before the
 * matching D1 migration is applied. The checked-in migration remains canonical;
 * these idempotent statements are only a production safety net.
 */
export const ensurePinterestEngagementSchema = (database: D1Database): Promise<void> => {
  if (!engagementSchemaPromise) {
    engagementSchemaPromise = database.batch([
      database.prepare(
